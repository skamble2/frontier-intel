# Evaluation report

Generated by `python3 -m fli.cli evaluate`. Every number and figure here is reproducible by re-running that one command — nothing is hand-copied.

**Metric tiers.** F1/precision/recall are only reported where ground truth exists: known by construction (synthetic) or against a stated human reference. On real data with no gold standard this report gives AGREEMENT, never "accuracy". Each figure carries its tier.

## Corpus

| quantity | value |
|---|---|
| documents | 1855 |
| evidence spans | 2952 |
| insights (events) | 954 |
| clustered | 954 |
| feature rows | 19080 |
| pairwise labels | 2735 |
| scored events | 9540 |
| tracked labs | 8 |
| tracked people | 337 |
| LLM spend (USD) | 29.87 |

## Pairwise labelers

| labeler | judgements | ties |
|---|---|---|
| `llm:claude-sonnet-5/technical/r1` | 791 | 0 |
| `llm:claude-sonnet-5/investment/r1` | 755 | 0 |
| `llm:gpt-5.2/investment/r1` | 462 | 0 |
| `human:soham/investment/r1` | 363 | 40 |
| `llm:gpt-5.2/technical/r1` | 300 | 0 |
| `human:soham/technical/r1` | 64 | 11 |

## Figures

### Signal-vs-noise funnel

![Signal-vs-noise funnel](figures/f1_funnel.png)

*MECHANICAL — arithmetic over the database, no labels needed*

1458 content documents fetched; 372 survived filtering (25.5%), yielding 954 events (2.56 per surviving document). 397 register document(s) (identity-evidence pages) are excluded from every bar — they are fetched to prove who someone is, not to yield insights.

### Feature correlation

![Feature correlation](figures/f2_feature_correlation.png)

*MECHANICAL — arithmetic over the database, no labels needed*

20 features over 954 events; most correlated pair: source_type_blog / source_type_newsroom.

### Event types per lab

![Event types per lab](figures/f3_event_types_by_lab.png)

*MECHANICAL — arithmetic over the database, no labels needed*

most common: release (290); rarest: personnel (10).

### Cost per task

![Cost per task](figures/f4_cost_by_task.png)

*MECHANICAL — arithmetic over the database, no labels needed*

$29.87 total across 8707 calls.

### Lexicon vs LLM classifier

![Lexicon vs LLM classifier](figures/f5_lexicon_vs_classifier.png)

*vs HUMAN REFERENCE — agreement, not accuracy*

keyword lexicon F1=0.267 · LLM classifier F1=0.444 — scored on 100 labels, all human-audited, under policy v3. Agreement with a human-audited reference.

### Labeler reliability (Dawid-Skene)

![Labeler reliability (Dawid-Skene)](figures/f6_labeler_reliability.png)

Rubric `investment` only: 462 pairs x 3 labelers across 3 independent model families (human:soham, llm:claude-sonnet-5, llm:gpt-5.2); pairs voted by a single family are excluded, since they carry no disagreement to learn from. Estimated accuracy: human:soham/investment/r1 0.778, llm:claude-sonnet-5/investment/r1 0.864, llm:gpt-5.2/investment/r1 0.874. Reliability is inferred from disagreement between families, so the figure is not produced from a single family — and labelers working to a DIFFERENT rubric are excluded, since they are estimating a different truth and would be scored as unreliable for disagreeing. The human's estimate is depressed by adverse selection, not skill: human votes come from the audit, disagreement and near-tie queues — deliberately the hardest pairs — while the LLM labelers vote on the whole random sample, so the two numbers are not on the same difficulty scale.

### Bake-off

![Bake-off](figures/f7_bakeoff.png)

760 labels, 224 held-out pairs; winner logistic at 0.768 held-out accuracy (chance is 0.5; every model must beat the baselines shown grey to earn its complexity).

### Feature ablation

![Feature ablation](figures/f8_ablation.png)

features that matter: source_type_arxiv, source_type_github, mechanism_channel, specificity.

### Per-lab fairness

![Per-lab fairness](figures/f9_per_lab_fairness.png)

raw p@10 spread 0.300, but raw p@10 tracks each lab's base rate, so LIFT is the fairness number: Google DeepMind +0.60, Anthropic +0.53, OpenAI +0.42, Qwen +0.35, DeepSeek +0.34, Mistral +0.32, Meta AI +0.30. Weakest lift: Meta AI (+0.30 on a 0.40 base, n=40) — its top-10 misses are infrastructure x2, release x1: events whose feature shape (official-channel engineering posts, high specificity) the score rewards but the judges call irrelevant. A feature-shape gap, not lab-identity bias (lab is never a feature). Lift is also CEILINGED at 1-base, so the fair per-lab comparison is lift/ceiling: Google DeepMind 100%, Anthropic 84%, OpenAI 58%, Qwen 54%, DeepSeek 100%, Mistral 52%, Meta AI 50%.

### Overfitting (train vs held-out)

![Overfitting (train vs held-out)](figures/f10_overfitting.png)

largest gap gbm_sklearn +0.108 — MEMORISING: held-out accuracy is not evidence of generalisation.

### Learning curve

![Learning curve](figures/f11_learning_curve.png)

last step 160->320 pairs moved accuracy +0.006; plateaued — more labels will NOT help; the features are the limit.

### Rank skew by lab

![Rank skew by lab](figures/f12_rank_skew.png)

most over-represented: DeepSeek at 3.1x its corpus share (4% of events, 12% of the top 50). Not rated: xAI (<10 events; a ratio on so few is noise).

### Rubric divergence

![Rubric divergence](figures/f13_rubric_divergence.png)

`investment` and `technical` share 0 of their top 10 (8% of the top 25) and rank the corpus at Kendall tau +0.215 — near zero, i.e. the two orderings are close to unrelated. Same events, same features, same clustering; only the definition of 'important' differs. This is the measurement behind the claim that one ranking cannot serve both readers.

### Talent mobility mechanism

![Talent mobility mechanism](figures/f14_mobility.png)

*MECHANICAL — arithmetic over the database, no labels needed*

10 personnel event(s) came from document extraction; 0 were synthesized from affiliation history (a person observed at two labs in succession). 193 of 337 tracked people are re-observable (23 via lab pages, 40 via X bios on a weekly cadence), and 337 were observed inside the last 30 days. The mechanism itself is validated end-to-end in the test suite (tests/knowledge/test_mobility.py plants a move and shows the resulting event reach the digest slate, dated by its arrival) — the live corpus simply has not yet witnessed a move. Profile re-observation runs on a 7-day cadence and the most recent observation landed 2026-08-05, so the earliest date a cadence-gated move can be witnessed live is 2026-08-12 — the zero here is arithmetic, not omission. RANK PROBE — synthetic, in-memory, nothing written: a fresh move carrying the median feature shape of the corpus's 10 extracted personnel events, dated two days ago and scored by the same antisymmetric-logistic path the bake-off ships, would rank investment #39 of 954 (top 4%); technical #954 of 954. That the two rubrics disagree is the design working: a researcher move is an investment signal, not a technical one.

### Claim faithfulness (entailment)

![Claim faithfulness (entailment)](figures/f15_faithfulness.png)

Every quote is byte-verified against its source (check C2), so this measures the remaining hallucination surface: whether the extracted claim is supported by the quote ALONE. claude-haiku-4-5-20251001: 930/954 entailed (97.5%), 10 partial, 14 not entailed. `partial` names a load-bearing fact (a number, a date, an actor) the quote does not carry — the actionable failure mode for an extraction prompt revision. That revision has since been made and measured (see docs/prompts.md): a quote-first instruction block cut the partial rate on the 30 hardest documents from 65.6% to 47.9% in a seeded A/B re-extraction; it applies to new extractions, so the corpus numbers above are the pre-revision audit. Verification tier ledger: 954/954 insight quotes are exact-tier (byte-verbatim under the shared normalization; C19 enforces this stays 100%). The corpus's 1631 structural rows carry no claims — they are the register's author-name spans in structured documents, where structural IS the designed tier. Of the 52 quotes the verifier REJECTED (never shown to a reader): 25 are >=80%-contiguous near-misses — the verifier being stricter than the normalization, not model failures; 25 are paraphrases; only 2 are fabrications with no anchor in the source. The floor metric is really a characterized rejection ledger.

### Digest slate precision@k

![Digest slate precision@k](figures/f16_slate_precision.png)

*vs HUMAN REFERENCE — agreement, not accuracy*

Each delivered digest item was marked keep or cut by a human reader (`digest --review`), which is precision@k for the system's central question. ai_team: 12/14 kept (86%); investment: 10/19 kept (53%). A cut item names the noise the slate rules let through; the review is per persona because the two audiences call different things noise. The investment cuts are the failure class f9 diagnosed — vendor case studies and official-channel engineering posts whose feature shape (official source, high specificity) the score rewards but the reader rejects — so the two figures corroborate each other from independent human reads.

### Planted-policy recovery

![Planted-policy recovery](figures/f17_synthetic_recovery.png)

*SYNTHETIC — ground truth known by construction*

Four known weight vectors of increasing difficulty are planted over the real standardized feature matrix (real, so recovery must survive the actual feature correlations), 400 pairwise labels are generated from each with 10% of verdicts flipped — the same order of unreliability f6 measured in the real judge — and the bake-off's own training path (antisymmetric pair differences, intercept-free logistic) is asked for the planted ranking back. single_feature: AUC 1.00, F1 0.95; hand_shape: AUC 0.97, F1 0.93; dense_mixed: AUC 0.96, F1 0.88; anti_prior: AUC 0.98, F1 0.92. `anti_prior` puts a NEGATIVE weight on recency, so no baseline shape recovers it by luck; its recovery shows the machinery follows the labels, not the priors. This validates the machinery, not the product: it says that IF reader preferences are near-linear in these features, the pipeline finds them through judge-level noise — whether the judge's preferences are the READER's is what f6 and f16 measure.
