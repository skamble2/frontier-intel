...
1559 content documents fetched; 372 survived filtering (23.9%), yielding 954 events (2.56 per surviving document). 537 register document(s) (identity-evidence pages) are excluded from every bar — they are fetched to prove who someone is, not to yield insights.
...
760 labels, 224 held-out pairs; winner logistic at 0.772 held-out accuracy (chance is 0.5; every model must beat the baselines shown grey to earn its complexity).

### Feature ablation

![Feature ablation](figures/f8_ablation.png)

features that matter: corroboration, source_type_github, source_type_arxiv, specificity, mechanism_channel.
...
largest gap gbm_sklearn +0.110 — MEMORISING: held-out accuracy is not evidence of generalisation.
