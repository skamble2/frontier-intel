...
call). For most of the corpus's history they sat idle: the early judge runs
wrote cache tokens whose 5-minute ephemeral TTL expired between calls, paying
the 1.25× write premium and collecting none of the 0.10× read discount — a
small net loss that was reported here rather than quietly dropped. The most
recent runs turned both on: the latest judge, verification and repair tranches
went through the batch API at a flat 50% off, and the committed DB now shows
6,915 cache-write tokens against **65,178 cache-read tokens** — the read
discount finally collected (roughly $0.18 saved against half a cent of
premium). The remaining gap is historical: the majority of the judge line was
paid at full price before the batch path carried its first tranche, which is
why batching-by-default is carried into
[future scope](final-report.md#future-scope).
...
The budget for this case study was €100. **$29.87 was spent — under a third of
